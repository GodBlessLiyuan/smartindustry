    ...
    public ResultVO storage(MaterialStorageDTO dto) throws Exception {
        ...
        for (MaterialStorageDTO.StorageDetailDTO detailDTO : dto.getDetail()) {
            ...
            for (PrintLabelPO labelPO : labelPOs) {
                labelPO.setType(dto.getType());
                labelPO.setStatus(ReceiptConstant.LABEL_STORAGE_FINISH);
        ...
        // 操作记录
        recordMapper.insert(new RecordPO(null, dto.getRbid(), 1L, "夏慧", ReceiptConstant.RECORD_TYPE_STORAGE_CONFIRM, new Date(), ReceiptConstant.RECEIPT_MATERIAL_STORAGE));
